...
export default function Stepper({
  ...
  return (
    <div className="stepper-outer-container" {...rest}>
      <div className={`stepper-card-container ${stepCircleContainerClassName}`}>
          ...
          <div className={`stepper-footer-container ${footerClassName}`}>
            <div className={`stepper-footer-nav ${currentStep !== 1 ? 'spread' : 'end'}`}>
              ...
              <button 
                ...
                {...nextButtonProps}
              >
                {isLastStep ? 'Complete Initialization' : nextButtonText}
              </button>
            </div>
          </div>
        )}
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }) {
  ...
  return (
    <motion.div onClick={handleClick} className="stepper-step-indicator" animate={status} initial={false}>
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: 'rgba(255,255,255,0.05)', color: '#666', border: '1px solid rgba(255,255,255,0.1)' },
          active: { scale: 1.1, backgroundColor: '#d4af37', color: '#000', border: '1px solid #d4af37' },
          complete: { scale: 1, backgroundColor: 'rgba(212,175,55,0.2)', color: '#d4af37', border: '1px solid #d4af37' }
        }}
